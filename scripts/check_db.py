...
This scripts checks if a collection of implicit constraints inside a
database are satisfied.

The checks useful to verify if a pipeline has executed
successfully. For example, conside that there are 5 samples in your
data called ``sample1`` to ``sample5``. The pipeline creates gene
lists for each sample and stores them in table called ``sample1_fpkm``
to ``sample5_fpkm``. At the same time, a table ``read_counts`` is created
with a column for each sample. To check, if all is present, type::
